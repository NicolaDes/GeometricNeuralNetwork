...
void NN::backpropagation(Vector<type> &input, Vector<type> &expected_output)
{
    if (input.size() != X.size() || expected_output.size() != Y.size()) {
        std::cerr << "BACKPROPAGATION FAILURE: Inputs dimensions or output "
                     "dimensions do not match -> input: "
                  << input.size() << "==" << X.size()
                  << ", output: " << expected_output.size() << "==" << Y.size()
                  << "\n";
        std::exit(EXIT_FAILURE);
    }
    feedforward(input);
...
void NN::train(Vector<Vector<type>> &inputs, Vector<Vector<type>> &outputs,
               type alpha)
{
...
void NN::loadWeights(const std::string &file)
{
    ...
    if (rows != W1.rows() || cols != W1.cols()) {
        std::cerr << "LOADING WEIGHTS 1: Inputs dimensions do not match with "
    ...
    if (rows != W2.rows() || cols != W2.cols()) {
        std::cerr << "LOADING WEIGHTS : Inputs dimensions do not match with "
                     "neural network initialization!\n";
        std::exit(EXIT_FAILURE);
    }
    for (size_t i = 0; i < rows; ++i) {
