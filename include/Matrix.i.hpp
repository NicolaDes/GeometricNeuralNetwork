...
{
    this->values_ = new T[this->size_](); //without () the value is initialized!
    this->leading_ = cols;
...
template <typename T>
Matrix<T> Matrix<T>::operator()(T (*fptr)(T val)) const
{
...
Matrix<T> Matrix<T>::operator+(const Matrix<T> &m)
{
    ...
    {
        for (size_t j = 0; j < this->leading_; ++j)
            ret[i][j] = this->values_[i * this->leading_ + j] + m.values_[i * this->leading_ + j];
    }
    return ret;
}
    ...
    for (size_t i = 0; i < this->row_offeset_; ++i)
    {
        for (size_t j = 0; j < m.leading_; ++j)
        {
            for (size_t k = 0; k < this->leading_; ++k)
                ret[i][j] += this->values_[i * this->leading_ + k] * m.values_[k * m.leading_ + j];
        }
    }
    return ret;
}
...
Matrix<T> Matrix<T>::operator,(const Matrix<T> &m)
{
    ...
    {
        for (size_t j = 0; j < this->leading_; ++j)
            ret[i][j] = this->values_[i * this->leading_ + j] * m.values_[i * m.leading_ + j];
    }
    return ret;
}
} // namespace laobject